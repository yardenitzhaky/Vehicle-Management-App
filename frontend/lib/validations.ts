import { Vehicle, VehicleStatus, ValidationError } from '@/types/vehicle';
...
export function canSetToMaintenance(
  ...
  // At least 1 vehicle can be in maintenance, otherwise 5% of total (rounded down)
  const maxMaintenance = Math.max(1, Math.floor(totalVehicles * 0.05));
